...
# 长期记忆的「沉淀进度」：记着已经把记忆更新到哪一天（YYYY-MM-DD）。
# 每天首次启动（以及之后每小时一次）会检查它，把这一天之后的
# 对话 + 活动沉淀进 user_memory.md。放在 memory/ 下，跟对话流水账同处。
DIGEST_MARK = os.path.join(MEMORY_DIR, ".last_digest")
...
# ⚠️ 改完这个 DEFAULT 记得同步 config.example.json（那份是给用户抄的模板，
# 内容必须等于 DEFAULT 把 api_key 留空；不对齐的话新用户会照着一份旧提示词配置）。
# 仓库根目录有现成的重新生成命令，见 HANDOVER.md。
DEFAULT = {
    # ⚠️ 这个模型**必须能看图** —— 阿拾的核心就是看屏幕。
    # 填纯文本模型不会崩，但接口会返回「does not support image input」，等于瞎了。
    # DeepSeek 官方现在分两个模型（2026-09 查的官方文档）：
    #   deepseek-flash    → DeepSeek-V4.1-Flash，**支持视觉**，也是最便宜的一档
    #   deepseek-v4-pro   → 不支持图片，别填
    # 旧名 deepseek-v4-flash-vision-exp 仍可调用，但已退役、由 V4.1-Flash 承接。
    # 想换别家视觉模型见 README 的「模型必须能看图」那节。
    "api": {
        "base_url": "https://api.deepseek.com/v1",
        "api_key": "",
        "model": "deepseek-flash",
        "vision_model": "",     # 留空 = 聊天和看图共用上面那个 model
        "temperature": 0.8,
        "max_tokens": 1024,
    },
    "pet": {
        "name": "阿拾",
        # 它怎么称呼**用户**（不是它自己的名字）。用来做沉浸感：
        # 「这么多的代码是想把{user}累坏吗」这种替他发声的口气。
        # 提示词里用 {user} 占位符引用，改完设置里所有提示词自动跟着变。
        "user_name": "宝宝",
        "x": -1,
        "y": -1,
        "scale": 1.0,
    ...
    "sense": {
        ...
        "prompt": (
            ...
            "画面里的文字就是内容——聊天记录、代码、弹幕、字幕、文档、"
            "别人发来的消息、输入框里已经打出来的字，都要当内容读："
            "可以引用其中一句、一个词、一个错别字、一个离谱的需求来说事。"
            "要谈的是「这些字/这事儿」，不是「用户在打字」这件事，"
            "也不要对着他说话。"
            "硬规则（必须遵守）："
            ...
            "⑤ 屏幕上那个光标/鼠标指针不算内容——不许拿它当话题，"
            "不许说它停着、转圈、卡住了；要谈就谈画面里真正的角色、文字、"
            "界面、颜色、剧情。"
            "反例——下面这种口气绝对不要，一个都不许："
    ...
    "persona": (
        ...
        "也不用「加油」「记得」「建议」这种口气。"
        "绝不毒舌、不讽刺、不阴阳怪气。"
        "你习惯叫他「{user}」——提到他的时候用这个称呼，别满口「你」「您」，"
        "那样像客服在跟他说话。你跟他是一伙的，是他的嘴替："
        "他忙得顾不上说话的时候，你替他把话说了——替他喊累、替他不平、"
        "替他吐槽那个离谱的需求（「这么多代码是想把{user}累坏吗」就是这个意思）。"
        "记住「嘴替」是**站在他这边帮他发声**，不是关心他、不是给建议、"
        "更不是问他问题。"
    ),
}
...
def render(text, cfg):
    """把提示词里的 {name} 换成宠物名、{user} 换成「对用户的称呼」。

    做成占位符而不是写死，是为了让用户在设置里改完名字/称呼之后，
    所有提示词自动跟着变，不需要重新生成整段提示词。
    """
    pet = (cfg or {}).get("pet") or {}
    name = pet.get("name") or DEFAULT["pet"]["name"]
    user = pet.get("user_name") or DEFAULT["pet"]["user_name"]
    return (text or "").replace("{name}", name).replace("{user}", user)
